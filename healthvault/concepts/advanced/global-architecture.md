...
A single-instance web child app is provisioned in an instance by using HealthVault app provisioning APIs like the [AddApplication](/healthvault/reference/methods/addapplication) and [UpdateApplication](/healthvault/reference/methods/updateapplication) requests (or by using one of the SDK utilities like the .NET [Provisioner](https://docs.microsoft.com/en-us/dotnet/api/microsoft.health.applicationprovisioning.provisioner) class). A multi-instance web child app is similarly provisioned, but must be done so in each instance the child app supports. For more information about multi-instance applications, see [master and child applications](/healthvault/concepts/advanced/master-and-child-applications).
...
HealthVault provides the [SelectInstance](/healthvault/reference/methods/selectinstance) request to help applications make this determination. The method takes the user's country/region as the request input, and returns the instanceID for the HealthVault instance where users from this country/region are mapped.
